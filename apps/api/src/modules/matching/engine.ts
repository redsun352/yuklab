...
export async function findMatches(prisma: PrismaClient, orderId: string): Promise<MatchCandidate[]> {
  ...
  const candidates = await Promise.all(providers.map(async (provider): Promise<MatchCandidate | null> => {
    ...
    return { providerId: provider.id, providerRole: provider.role as "DRIVER" | "SERVICE_PROVIDER", providerCategory: category, score, distanceKm, rating, reliabilityScore: reliability, etaMinutes, vehicleId: vehicle?.id ?? null, vehicleType: vehicle?.type ?? null, vehicleSubtype: vehicle?.subtype ?? null, capacityKg: vehicle?.capacityKg == null ? null : Number(vehicle.capacityKg), volumeM3: vehicle?.volumeM3 == null ? null : Number(vehicle.volumeM3), refrigerated: vehicle?.refrigerated ?? false };
  }));

  return candidates.filter((candidate): candidate is MatchCandidate => candidate !== null).sort((a, b) => b.score - a.score);
}

export async function invalidateMatchingCache(orderId: string): Promise<void> {
  if (!redis) return;
  try {
    if (redis.status !== "ready") await redis.connect();
    await redis.del(`yuklab:matching:order:${orderId}`);
  } catch {
    // Matching remains database/GPS driven when Redis is unavailable.
  }
}
